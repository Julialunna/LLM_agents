...
#shared state
class SimplesAgentState(TypedDict):
    original_query: str
...
#simple planner, divides on calculus and explanation
def planner_node(state:SimplesAgentState)-> Dict[str, Any]:
    ...
    plan=[
        ...
        },
        {
            ...
            "description": f"Resolva em linguagem simples o resultado do cálculo feito na tarefa 'do_math'."
        }
    ]
    return{
        "plan":plan, 
...
def prepare_next_task_node(state: SimplesAgentState) -> Dict[str, Any]:
    plan= state.get("plan", [])
...
def mathematician_node(state:SimplesAgentState) -> Dict[str, str]:
...
def writer_node(state: SimplesAgentState)-> Dict[str, str]:
    prev_results = state.get("intermediate_results", {})
    math_result = prev_results.get("do_math", "sem resultado")
    return {"specialist_result":f"O resultado do cálculo é {math_result}. Isso significa que, \
            ao resolver a expressão chegamos a esse valor."}
    
def collect_result_and_advance_node(state: SimplesAgentState) -> Dict[str, str]:
...
def systhesis_node(state:SimplesAgentState) -> Dict[str, str] | None:
...
def should_execute_task_or_sythesize(state:SimplesAgentState) -> str:
    if state.get("error"):
...
def specialist_router_node(state: SimplesAgentState) ->str:
...
def error_node(state: SimplesAgentState)->Dict[str,str]:
...
workflow_builder = StateGraph(SimplesAgentState)
